...
import { selectMyTimesheets } from '../../timesheets/store/selectors';
...
export default function AddTimeLogModal({
  ...
  const selectedEmployee = useMemo(
    () => employeeOptions.find(e => e.value === employeeId),
  ...
  useEffect(() => {
    ...
    setError('');
  }, [visible, editData, fixedProjectId]);

  useEffect(() => {
    if (!visible) return;

    // EDIT MODE → always trust backend data
    if (editData?.employeeId) {
      setEmployeeId(editData.employeeId);
      return;
    }

    // ADD MODE → set default ONLY if not already selected
    if (!employeeId && projectMembers?.length) {
      setEmployeeId(projectMembers[0].employeeId);
    }
  }, [visible, editData, projectMembers, employeeId]);
      ...
      <View style={styles.backdrop}>
        <View style={styles.sheet}>
          ...
          <ScrollView contentContainerStyle={styles.body}>
            ...
            >
              <Text style={{ color: selectedEmployee ? '#111' : '#9ca3af' }}>
                {selectedEmployee?.label || 'Select Employee'}
              </Text>
            </Pressable>
            ...
            <BottomSheetSelect
              ...
              onSelect={o => setEmployeeId(o.value)}
