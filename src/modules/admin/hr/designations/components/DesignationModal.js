...
export default function DesignationModal({
  ...
  useEffect(() => {
    if (editing) {
      setName(editing.designationName || '');
      setParentId(editing.parentDesignationId ?? null);
    } else {
      setName('');
      setParentId(null);
    }
  ...
  const parentOptions = useMemo(() => {
    ...
    list
      .filter(x => x.id !== editing?.id) // cannot parent to itself
      .forEach(x =>
        opts.push({ label: `${x.designationName} (#${x.id})`, value: x.id }),
      );
    return opts;
  }, [list, editing]);
  ...
  const submit = () => {
    if (!name.trim()) {
      Alert.alert('Missing', 'Designation name is required');
      return;
    }
    onSave({
