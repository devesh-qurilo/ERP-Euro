...
export default function DepartmentsTable({
  ...
  const widths = [120, 240, 200, 160, 140];
  const cols = ['#ID', 'Department', 'Parent', 'Created', 'Action'];
        ...
        {(loading ? [] : data).map(row => {
          ...
          return (
            <Row key={row.id}>
              ...
              <Cell w={widths[1]} text={row.departmentName} />
              <Cell
                w={widths[2]}
                text={
                  row.parentDepartmentName ||
                  (row.parentDepartmentId ? `#${row.parentDepartmentId}` : '—')
                }
              />
              <Cell w={widths[3]} text={row.createAt || '—'} />
              <Cell w={widths[4]}>
