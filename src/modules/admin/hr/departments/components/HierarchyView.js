    ...
    <ScrollView style={styles.wrap} contentContainerStyle={{ padding: 8 }}>
      {items.map(node => {
        const parentLabel =
          ...
          (node.parentDepartmentId ? `#${node.parentDepartmentId}` : 'None');
        return (
          <View key={node.id} style={styles.item}>
            <View style={{ flex: 1 }}>
              <Text style={styles.title}>{node.departmentName}</Text>
              <Text style={styles.dim}>
                ID: #{node.id} • Created: {node.createAt || '—'}
              </Text>
            </View>
            <View style={{ width: 12 }} />
            <View style={{ minWidth: 160 }}>
              ...
              <Select
                value={parentLabel}
                options={options.filter(o => o.value !== node.id)}
      ...
      {!items.length && <Text style={styles.dim}>No departments</Text>}
    </ScrollView>
  );
}
